...
OTA_MAX_CFO_ERROR_HZ = 300  # relaxed for combined phase noise + SCO stress test
...
def _run_sync(rx, long_ref):
    """Run coarse + fine and return (coarse, fine) or (coarse, None) if coarse fails."""
...
def test_sync_pipeline(cfo_hz: int, channel: SyncFixture) -> None:
    ...
    for seed in range(N_TRIALS):
        ...
        coarse, fine = _run_sync(rx, channel.long_ref)
        if fine is None:
            continue
        detects += 1
    ...
    for s in range(N_FP_TRIALS):
        ...
        fp = coarse_sync(noise, SAMPLE_RATE, SPS, SYNC_CFG)
    ...
    fm = fine_timing(buf, channel.long_ref, cm.d_hats[:1], cm.cfo_hats[:1], SAMPLE_RATE, SPS, SYNC_CFG)
    ...
    cp = coarse_sync(rx[SAMPLE_OFFSET + half:], SAMPLE_RATE, SPS, SYNC_CFG)
    ...
    co, fo = _run_sync(rx, channel.long_ref)
    ...
    for test_cfo_hz in [0, 500, 2000]:
        for test_phase_rad in [0, np.pi / 2, -np.pi * 3 / 4]:
            ...
            payload_pos = fine.sample_idxs[0] + len(channel.long_ref)
