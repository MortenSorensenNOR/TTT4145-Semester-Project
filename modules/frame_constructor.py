...
from .channel_coding import LDPC, CodeRates, Golay, LDPCConfig
...
class FrameHeaderConfig:
    ...
    def __post_init__(self) -> None:
        """Compute the total header size from field widths."""
...
class FrameHeaderConstructor:
    ...
    def __init__(self, config: FrameHeaderConfig | None = None) -> None:
        ...
        config = config or FrameHeaderConfig()
    ...
    def decode(self, header: np.ndarray) -> FrameHeader:
        ...
        data_str = "".join(
            str(b)
            for b in (length_bits + src_bits + dst_bits + mod_scheme_bits + coding_rate_bits + [0] * self.reserved_bits)
        )
...
class FrameConstructor:
    ...
    def encode(self, header: FrameHeader, payload: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        payload_padded = np.concatenate(
            [
                payload,
                np.zeros(k - header.length, dtype=int),
            ],
        )
    ...
    def decode(
        ...
        is_llr = not np.all(
            (np.abs(payload_encoded) <= 1) | (np.isclose(payload_encoded, 0)) | (np.isclose(payload_encoded, 1)),
        )

        # LDPC uses soft decision - convert bits to LLR if needed
        payload_llr = payload_encoded if is_llr else 10.0 * (1 - 2 * payload_encoded.astype(np.float64))
