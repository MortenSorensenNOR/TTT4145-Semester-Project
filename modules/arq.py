...
Two bytes of bitmap cover any ``window_size`` ≤ 16; for windows beyond that
the receiver still SACKs the first 16 out-of-order seqs and the sender falls
back to retransmitting the rest on timeout.
...
Sequence space: 7 bits → 0..127; window size must be < SEQ_SPACE/2 (= 64) so
that circular-distance comparisons stay unambiguous. The width has to match
the on-wire frame header (``FrameHeaderConfig.sequence_number_bits``); if
ARQ tried to send a seq beyond that range the encoder would silently
truncate it and the receiver would treat the frame as a stale duplicate,
breaking all forward progress.
...
SEQ_SPACE: int = 128  # 7-bit sequence numbers: 0 .. 127
...
@dataclass
class ARQConfig:
    window_size: int = 63              # max unacked in-flight frames (< SEQ_SPACE/2 = 64)
