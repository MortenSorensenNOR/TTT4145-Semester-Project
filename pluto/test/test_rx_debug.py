...
def main() -> None:
    ...
    try:
        while True:
            ...
            result = sync.detect_preamble(filtered, np.float32(SAMPLE_RATE))
            ...
            if len(filtered) >= short_len:
                ...
                corr = np.abs(np.correlate(filtered[: short_len * 4], np.repeat(short_zc, SPS).astype(np.complex64), mode="valid"))
                ...
                max_corr / (np.sqrt(sig_power * ref_power) * len(short_zc) * SPS + np.float32(1e-12))
