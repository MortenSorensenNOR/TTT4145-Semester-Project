...
# ─── Helpers ──────────────────────────────────────────────────────────────────
freqs_mhz = (np.fft.fftshift(np.fft.fftfreq(FFT_SIZE, d=1/SAMPLE_RATE))
             + CENTER_FREQ) / 1e6
...
def bandpass_around_tone(samples, offset_hz, bw=20e3):
...
line_live, = ax_spec.plot(freqs_mhz, np.full(FFT_SIZE, -120.0),
                           color="#00d4ff", lw=0.8, alpha=0.4, label="Live")
line_avg,  = ax_spec.plot(freqs_mhz, np.full(FFT_SIZE, -120.0),
                           color="#ff6b35", lw=1.4, label=f"Avg x{AVERAGING}")
line_peak, = ax_spec.plot(freqs_mhz, np.full(FFT_SIZE, -120.0),
                           color="#a8ff3e", lw=0.8, ls="--", alpha=0.7,
                           label="Peak hold")

ax_spec.axvline(tone_mhz, color="#ffdd00", lw=1.0, ls="--", alpha=0.8,
                label=f"Expected {tone_mhz:.3f} MHz")
ax_spec.axvline(CENTER_FREQ / 1e6, color="white", lw=0.5, ls=":", alpha=0.3)
ax_spec.legend(loc="upper right", framealpha=0.25,
               facecolor="#1a1d24", labelcolor="white", fontsize=8)

stats_text = ax_spec.text(
    0.01, 0.97, "", transform=ax_spec.transAxes,
    color="#cccccc", fontsize=8, va="top", fontfamily="monospace",
...
wf_img = ax_wf.imshow(waterfall_data, aspect="auto", origin="upper",
                       extent=[freqs_mhz[0], freqs_mhz[-1], WATERFALL_ROWS, 0],
                       vmin=-120, vmax=0, cmap="inferno", interpolation="nearest")
...
iq_scatter = ax_iq.scatter([], [], c=[], cmap="plasma", s=4,
...
line_i, = ax_time.plot(t_axis_us, np.zeros(SHOW_SAMPLES),
                        color="#00d4ff", lw=1.0, label="I")
line_q, = ax_time.plot(t_axis_us, np.zeros(SHOW_SAMPLES),
                        color="#ff4dff", lw=1.0, label="Q")
ax_time.legend(loc="upper right", framealpha=0.2,
               facecolor="#1a1d24", labelcolor="white", fontsize=8)
...
def update(_frame):
    ...
    filtered = bandpass_around_tone(raw, detected_offset_hz, bw=20e3)
    ...
    spectrum[np.abs(lp_freqs) > 10e3] = 0
    ...
    return (line_live, line_avg, line_peak, wf_img, stats_text,
            iq_scatter, line_i, line_q)
