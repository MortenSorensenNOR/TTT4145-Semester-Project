...
def tx_thread(tun_fd: int, sdr: object, mtu: int) -> None:
    ...
    # Pre-warm LDPC cache for all payload sizes to avoid 100-400ms JIT delays
    ...
    # Warm multi-block paths (2, 3, 4+ blocks) - common packet sizes
    max_k = ldpc_max_k(CODING_RATE)
    for n_blocks in range(2, 9):  # 2-8 blocks
        dummy_bits = np.zeros((n_blocks - 1) * max_k + 100, dtype=np.uint8)
        if len(dummy_bits) <= max_bits:
            build_tx_signal_from_bits(dummy_bits, MOD_SCHEME, CODING_RATE)
...
def rx_thread_bridge(tun_fd: int, sdr: object) -> None:
    ...
    # Pre-warm LDPC decode cache (all block sizes)
