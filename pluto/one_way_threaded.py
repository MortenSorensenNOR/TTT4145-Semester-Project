"""One-way TX or RX loopback test.

Run two instances on the two nodes — one in --mode tx, one in --mode rx —
to verify the link end-to-end. Per-node TX/RX IP assignment is loaded from
pluto/setup.json; see pluto.setup_config for the schema.
"""
...
if __name__ == "__main__":
    ...
    tx_uri = setup.tx_uri(args.node) if args.mode == "tx" else None
    rx_uri = setup.rx_uri(args.node) if args.mode == "rx" else None

    # RX-LO CFO offset: pull the measured value for this node from the
    # calibration, or 0 if missing. TX always emits at its natural LO
    # (split-radio convention — the peer's RX does the compensating).
    if args.mode == "tx":
        rx_cfo_hz = 0
        cfo_src   = "n/a (tx-only)"
    ...
    def _build_packet(seq: int, payload_bytes: int) -> np.ndarray:
        ...
        payload = _make_payload_bytes(seq, payload_bytes)
        ...
        try:
            while True:
                ...
                status.log(f"  [TX] burst {burst_num}: {args.packets} pkts "
                           f"(seq {burst_start}–{global_seq - 1}) sent in {t1 - t0:.3f}s  "
    ...
    def run_rx():
        ...
        try:
            while True:
                ...
                gain_str = f"  gain={args.rx_gain:>4.1f}dB" if args.rx_gain_mode == "manual" else ""
